...
export default class NetSuiteItemsList extends LightningElement {
  get columns() {
    return [
      ...
      {
        label: "Display Name",
        fieldName: "breadwinner_ns__DisplayName__c",
        type: "text"
      },
      {
      ...
      },
      {
        ...
        fieldName: "ncf_item_stoneweight1__c",
        type: "text"
      },
      {
        label: "Total Carat Weight",
        fieldName: "ncf_item_stdweight__c",
        type: "text"
      },
      { label: "Selected Location", fieldName: "Location", type: "text" },
      {
        label: "Location Qty Avail",
        fieldName: "LocationQtyAvail",
        type: "text"
      },
      {
        label: "Location On Hand",
        fieldName: "LocationOnHand",
        type: "text",
        sortable: true
      },
      {
        label: "Location Qty In Transit",
        fieldName: "LocationQtyInTransit",
        type: "text"
      },
      ...
      ...(this.showCostMarginFields
        ? [
            {
              label: "Cost",
              fieldName: "breadwinner_ns__Cost__c",
              type: "text",
              sortable: true
            },
            { label: "Margin %", fieldName: "Margin_Percent__c", type: "text" },
            {
              ...
              fieldName: "ncf_item_replacementcost__c",
              type: "text"
            }
          ]
        : []),
      ...
      {
        label: "Total Weight",
        fieldName: "Total_Weight_Displayed_in_Table__c",
        type: "text"
      },
      ...
      {
        label: "Division",
        fieldName: "breadwinner_ns__ClassName__c",
        type: "text"
      },
      {
        label: "Merchandise Dept",
        fieldName: "ncf_item_psgss_merc_dept__c",
        type: "text"
      },
      ...
      {
        label: "Is Serial",
        fieldName: "ncf_item_serialized_item_type__c",
        type: "text"
      },
      {
        label: "Watch Serial #",
        fieldName: "ncf_item_watchserial__c",
        type: "text"
      },
      {
        label: "Date First Receipt",
        fieldName: "ncf_item_datefirstreceipt__c",
        type: "text"
      },
      {
        label: "Date Last Receipt",
        fieldName: "ncf_item_datelastreceipt__c",
        type: "text"
      }
  ...
  @wire(getRecord, {
    recordId: USER_ID,
    fields: [USER_ROLE_NAME_FIELD, USER_PROFILE_NAME]
  })
  ...
  showNetSuiteItems(pageResult, isLoadedMore) {
    ...
    pageResult.netSuiteItems.forEach((item) => {
      ...
      dataItem.breadwinner_ns__Cost__c = this.formatCurrency(
        dataItem.breadwinner_ns__Cost__c
      );
      dataItem.ncf_item_replacementcost__c = this.formatCurrency(
        dataItem.ncf_item_replacementcost__c
      );
      ...
      if (this.filter.rollUpSubitemsChange && pageResult.rollUpQty) {
        ...
        const inv = dataItem.breadwinner_ns__BWInventoryLocations__r?.find(
          (loc) =>
            loc.breadwinner_ns__Location__r.Name ===
            this.filter.preferredLocation
        );

        if (inv) {
          dataItem.LocationQtyAvail =
            parseInt(inv.breadwinner_ns__QuantityAvailable__c) || 0;
          dataItem.LocationOnHand =
            parseInt(inv.breadwinner_ns__QuantityOnHand__c) || 0;
          dataItem.LocationQtyInTransit =
            parseInt(inv.breadwinner_ns__QuantityBackOrdered__c) || 0;
        } else {
    ...
    if (pageResult.showSubtotals) {
      ...
      let totalItem = {
        ...
        ncf_item_replacementcost__c: this.formatCurrency(
          this.subtotals.totalReplacementCost
        ),
